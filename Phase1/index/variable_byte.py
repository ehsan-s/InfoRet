class VariableByteCompressor:
    ...
    @staticmethod
    def compress_to_file(dictionary, file='var_index.txt'):
        f = open(file, "w")
        ...
        for t_id in sorted(dictionary):
            ...
            f.write(VariableByteCompressor.__compress_posting_list(doc_id_list) + '\n')
            for doc_id in doc_id_list:
                f.write(VariableByteCompressor.__compress_posting_list(posting_dict[doc_id]) + '\n')
...
class VariableByteDecompressor:
    ...
    @staticmethod
    def decompress_from_file(file='var_index.txt'):
        f = open(file, "r")
        dictionary = {}
        tid_num = int(f.readline())
        for i in range(tid_num):
            tid_dict = {}
            doc_list = VariableByteDecompressor.__decompress_posting_list(f.readline().strip('\n'))

            for doc_id in doc_list:
                posting_list = VariableByteDecompressor.__decompress_posting_list(f.readline().strip('\n'))
